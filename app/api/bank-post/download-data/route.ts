...
import type { DownloadDataRequest, DownloadDataResponse } from '../types';
import { API_URL } from '@/lib/utils';

// Backend API URL
const BACKEND_API_URL = `${API_URL}/api/bank-post/download-data`;

/**
 * POST /api/bank-post/overall-insights
 * 
 * Proxy endpoint for overall insights based on a topic
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { topic, keywords, bank, start_date, end_date } = body;

    // Validate the topic parameter
    if (!topic || !Array.isArray(topic)) {
      return NextResponse.json(
        { error: 'Topic parameter is required and must be an array' },
        { status: 400 }
      );
    }
    console.log('Forwarding topic to backend API:', topic);
    ...
    console.error('Error processing overall insights request:', error);
